...
public class LayersIceZertum implements LayerRenderer {
	...
	private final RenderIceZertum renderer;
		...
		if (!entity.isInvisible()) {
			if (!entity.hasEvolved()) {
				...
				if (entity.isTamed() && entity.getGender() == true) {
					this.renderer.bindTexture(ResourceReference.getZMaleTameLayers("i"));
					GlStateManager.color(1f, 1f, 1f);
					this.renderer.getMainModel().render(entity, par1, par2, par4, par5, par6, par7);
				}
				if (!entity.isTamed() && entity.getGender() == true) {
					this.renderer.bindTexture(ResourceReference.getZMaleLayers("i"));
					GlStateManager.color(1f, 1f, 1f);
					this.renderer.getMainModel().render(entity, par1, par2, par4, par5, par6, par7);
				}
			}
				...
				if (entity.getGender() == true) {
					if (entity.isTamed()) {
						this.renderer.bindTexture(ResourceReference.getZEMaleLayers("i"));
						GlStateManager.color(1f, 1f, 1f);
						this.renderer.getMainModel().render(entity, par1, par2, par4, par5, par6, par7);
					}
				}
				...
				if (entity.getGender() == true) {
					this.renderer.bindTexture(ResourceReference.getZFinalMaleLayers("i"));
					GlStateManager.color(1f, 1f, 1f);
					this.renderer.getMainModel().render(entity, par1, par2, par4, par5, par6, par7);
				}
			}
