...
public class ModVersionCheck {
	...
	private void verifyModVersion(EntityPlayer player) {
		try {
			...
			player.addChatMessage(ChatHelper.getChatComponent(EnumChatFormatting.RED + "The URL cannot be found, contact the developer for more information!"));
			status = FAILED;
		}
		catch (IOException e) {
			e.printStackTrace();
			player.addChatMessage(ChatHelper.getChatComponent(EnumChatFormatting.RED + "Could not verify version for " + name));
			player.addChatMessage(ChatHelper.getChatComponent(EnumChatFormatting.RED + "Check your internet connection or contact the developer!"));
			status = FAILED;
		}
