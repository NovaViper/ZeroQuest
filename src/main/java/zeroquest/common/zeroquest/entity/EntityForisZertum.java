...
{   
    ...
    public static final int INDEX_COLLAR_COLOR = 20;
    ...
    {
        this.dataWatcher.updateObject(INDEX_HEALTH, Float.valueOf(this.getHealth()));
    }
    ...
    public void onLivingUpdate()
    {
    	...
    	for(int l = 0; l <= 4; ++l) //TODO
        {
            ...
            if (this.worldObj.getBlock(i, j, k) == Blocks.dirt
            		 && this.worldObj.getBlockMetadata(i, j, k) == 0 // dirt not podzol
            		 && this.worldObj.getBlockLightValue(i, j+1, k) >= 4 
            		 && this.worldObj.getBlockLightOpacity(i, j+1, k) <= 2){
    	...
    	ParticleEffects.spawnParticle("forisdust", this.posX + (double)(this.rand.nextFloat() * this.width * 2.0F) - (double)this.width, this.posY + 0.5D + (double)(this.rand.nextFloat() * this.height), this.posZ + (double)(this.rand.nextFloat() * this.width * 2.0F) - (double)this.width, d0, d1, d2);
