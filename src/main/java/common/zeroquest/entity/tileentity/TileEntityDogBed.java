...
public class TileEntityDogBed extends TileEntity implements IUpdatePlayerListBox {
	...
	@Override
	public void update() {
		List dogs = this.worldObj.getEntitiesWithinAABB(EntityZertumEntity.class, new AxisAlignedBB(this.pos.getX(), this.pos.getY(), this.pos.getZ(), this.pos.getX() + 1.0D, this.pos.getY() + 1.0D, this.pos.getZ() + 1.0D).expand(3D, 2D, 3D));
		 
	    if (dogs != null && dogs.size() > 0) {
	    	for (int index = 0; index < dogs.size(); index++) {
	    		EntityZertumEntity dog = (EntityZertumEntity)dogs.get(index);
