...
public class BlockPortalDarkax extends BlockPortal {
	...
	public boolean func_176548_d(World worldIn, BlockPos p_176548_2_) {
		BlockPortalDarkax.Size size = new BlockPortalDarkax.Size(worldIn, p_176548_2_, EnumFacing.Axis.X);
		...
		if (size.func_150860_b() && size.field_150864_e == 0) {
			...
			BlockPortalDarkax.Size size1 = new BlockPortalDarkax.Size(worldIn, p_176548_2_, EnumFacing.Axis.Z);
	...
	public void randomDisplayTick(World worldIn, BlockPos pos, IBlockState state, Random rand) {
		...
		for (int i = 0; i < 4; ++i) {
			...
			if (worldIn.isRemote) {
				ClientProxy.spawnDarkParticle(worldIn, d0, d1, d2, d3, d4, d5);
			}
		}
	}
	...
	public void onNeighborBlockChange(World worldIn, BlockPos pos, IBlockState state, Block neighborBlock) {
		...
		BlockPortalDarkax.Size size;

		if (axis == EnumFacing.Axis.X) {
			size = new BlockPortalDarkax.Size(worldIn, pos, EnumFacing.Axis.X);
			...
			size = new BlockPortalDarkax.Size(worldIn, pos, EnumFacing.Axis.Z);
	...
	public static class Size {
		...
		protected int func_180120_a(BlockPos p_180120_1_, EnumFacing p_180120_2_) {
			...
			for (i = 0; i < 22; ++i) {
				...
				if (!this.func_150857_a(this.world.getBlockState(blockpos1).getBlock()) || this.world.getBlockState(blockpos1.down()).getBlock() != ModBlocks.darkaxStone) {
					break;
				}
			}

			Block block = this.world.getBlockState(p_180120_1_.offset(p_180120_2_, i)).getBlock();
			return block == ModBlocks.darkaxStone ? i : 0;
		}
		...
		protected int func_150858_a() {
			...
			for (this.field_150862_g = 0; this.field_150862_g < 21; ++this.field_150862_g) {
				for (i = 0; i < this.field_150868_h; ++i) {
					...
					if (block == ModBlocks.portalDarkax) {
						++this.field_150864_e;
					}
					...
					if (i == 0) {
						...
						if (block != ModBlocks.darkaxStone) {
							break label56;
						}
					}
			...
			for (i = 0; i < this.field_150868_h; ++i) {
				if (this.world.getBlockState(this.field_150861_f.offset(this.field_150866_c, i).up(this.field_150862_g)).getBlock() != ModBlocks.darkaxStone) {
		...
		protected boolean func_150857_a(Block p_150857_1_) {
			return p_150857_1_.getMaterial() == Material.air || p_150857_1_ == ModBlocks.darkFire || p_150857_1_ == ModBlocks.portalDarkax;
		}
		...
		public void func_150859_c() {
			for (int i = 0; i < this.field_150868_h; ++i) {
				...
				for (int j = 0; j < this.field_150862_g; ++j) {
					this.world.setBlockState(blockpos.up(j), ModBlocks.portalDarkax.getDefaultState().withProperty(BlockPortalDarkax.AXIS, this.axis), 2);
				}
			}
		}
	}
