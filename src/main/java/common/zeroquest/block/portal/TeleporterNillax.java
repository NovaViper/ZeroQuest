...
public class TeleporterNillax extends Teleporter {
				...
				for (int i1 = -2; i1 <= 2; ++i1) {
					for (int j1 = -1; j1 < 3; ++j1) {
						...
						this.worldServerInstance.setBlockState(new BlockPos(k1, l1, i2), flag
								? ModBlocks.nillaxStone.getDefaultState()
	...
	public boolean placeInExistingPortal(Entity entityIn, float p_180620_2_) {
		...
		if (d0 >= 0.0D) {
			if (flag1) {
				this.destinationCoordinateCache.add(k, new TeleporterNillax.PortalPosition((BlockPos) object, this.worldServerInstance.getTotalWorldTime()));
	...
	public boolean makePortal(Entity p_85188_1_) {
		...
		IBlockState iblockstate = ModBlocks.portalNillax.getDefaultState().withProperty(BlockPortalNillax.AXIS, l5 != 0
			...
			for (k3 = 0; k3 < 4; ++k3) {
				for (l3 = -1; l3 < 4; ++l3) {
					...
					this.worldServerInstance.setBlockState(new BlockPos(i4, j4, k4), flag1
							? ModBlocks.nillaxStone.getDefaultState() : iblockstate, 2);
				}
			}
