...
import common.zeroquest.client.model.ModelZertumEvo;
import common.zeroquest.client.model.ModelZertumFinal;
import common.zeroquest.client.render.layers.LayersIceZertum;
...
public class RenderIceZertum extends RenderLiving {
	...
	public RenderIceZertum(RenderManager render, ModelBase base, float par1) {
		...
		model2 = new ModelZertumEvo(); // or create new ones directly
		model3 = new ModelZertumFinal();
	}
	...
	protected ResourceLocation getEntityTexture(EntityIceZertum entity) {
		if (!entity.hasEvolved()) {
			if (entity.isTamed()) {
				return ResourceReference.getZTameSkins("i");
			}
			else if (entity.isAngry()) {
				return ResourceReference.getZAngrySkins("i");
			}
			else if (!entity.isTamed() && !entity.isAngry()) {
				return ResourceReference.getZWildSkins("i");
			}
		}
		else if (entity.hasEvolved()) {
			if (!entity.inFinalStage()) {
				return ResourceReference.getZEvoSkins("i");
			}
			else if (entity.inFinalStage()) {
				return ResourceReference.getZFinalSkins("i");
			}
		}
		return null;
	}
