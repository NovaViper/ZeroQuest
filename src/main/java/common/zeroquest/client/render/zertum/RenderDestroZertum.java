...
public class RenderDestroZertum extends RenderLiving {
	...
	protected ResourceLocation getEntityTexture(EntityDestroZertum entity) {
		if (!entity.hasEvolved()) {
			if (entity.isTamed()) {
				return ResourceReference.getZTameSkins("des");
			}
			else if (entity.isAngry()) {
				return ResourceReference.getZAngrySkins("des");
			}
			else if (!entity.isTamed() && !entity.isAngry()) {
				return ResourceReference.getZWildSkins("des");
			}
		}
		else if (entity.hasEvolved()) {
			if (!entity.inFinalStage()) {
				return ResourceReference.getZEvoSkins("des");
			}
			else if (entity.inFinalStage()) {
				return ResourceReference.getZFinalSkins("des");
			}
		}
		return null;
	}
	...
	public void doRender(EntityLiving entity, double par1, double par2, double par3, float par4, float par5) {
		EntityDestroZertum zertum = (EntityDestroZertum) entity;
