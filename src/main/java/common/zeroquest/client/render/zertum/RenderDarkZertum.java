...
public class RenderDarkZertum extends RenderLiving {
	...
	protected ResourceLocation getEntityTexture(EntityDarkZertum entity) {
		if (!entity.hasEvolved()) {
			if (entity.isTamed()) {
				return ResourceReference.getZTameSkins("d");
			}
			else if (entity.isAngry()) {
				return ResourceReference.getZAngrySkins("d");
			}
			else if (!entity.isTamed() && !entity.isAngry()) {
				return ResourceReference.getZWildSkins("d");
			}
		}
		else if (entity.hasEvolved()) {
			if (!entity.inFinalStage()) {
				return ResourceReference.getZEvoSkins("d");
			}
			else if (entity.inFinalStage()) {
				return ResourceReference.getZFinalSkins("d");
			}
		}
		return null;
	}
	...
	public void doRender(EntityLiving entity, double par1, double par2, double par3, float par4, float par5) {
		EntityDarkZertum zertum = (EntityDarkZertum) entity;
