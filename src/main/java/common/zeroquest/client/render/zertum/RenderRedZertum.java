...
import common.zeroquest.client.render.layers.LayersRedZertum;
...
public class RenderRedZertum extends RenderLiving {
	...
	protected ResourceLocation getEntityTexture(EntityRedZertum entity) {
		if (!entity.hasEvolved()) {
			if (entity.isTamed()) {
				return ResourceReference.getZTameSkins("r");
			}
			else if (entity.isAngry()) {
				return ResourceReference.getZAngrySkins("r");
			}
			else if (!entity.isTamed() && !entity.isAngry()) {
				return ResourceReference.getZWildSkins("r");
			}
		}
		else if (entity.hasEvolved()) {
			if (!entity.inFinalStage()) {
				return ResourceReference.getZEvoSkins("r");
			}
			else if (entity.inFinalStage()) {
				return ResourceReference.getZFinalSkins("r");
			}
		}
		return null;
	}
	...
	public void doRender(EntityLiving entity, double par1, double par2, double par3, float par4, float par5) {
		EntityRedZertum zertum = (EntityRedZertum) entity;
	...
	protected void renderOffsetLivingLabel(Entity entity, double x, double y, double z, String displayName, float scale, double distanceFromPlayer) {
		...
		EntityRedZertum dog = (EntityRedZertum) entity;
