...
public class EntityAIFollowOwner extends EntityAIBase {
    ...
    public EntityAIFollowOwner(EntityJakanPrime dragon, double speed, float minDist, float maxDist) {
        this.dragon = dragon;
        this.speed = speed;
    ...
    public boolean shouldExecute() {
        EntityLivingBase ownerCurrent = dragon.func_130012_q();
        ...
        for (int bx = 0; bx <= 4; ++bx) {
            for (int by = 0; by <= 4; ++by) {
                if ((bx < 1 || by < 1 || bx > 3 || by > 3)
                        && world.doesBlockHaveSolidTopSurface(minX + bx, minZ - 1, minY + by)
                        && !world.isBlockNormalCube(minX + bx, minZ, minY + by)
                        && !world.isBlockNormalCube(minX + bx, minZ + 1, minY + by)) {
