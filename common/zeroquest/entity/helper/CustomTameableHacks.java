 ...
 */
public class CustomTameableHacks extends CustomTameableHelper {
    ...
    public CustomTameableHacks(EntityCustomTameable tameable) {
        super(tameable);
...
/*        // override EntityBodyHelper field, which is private and has no setter
        ...
        try {
            ...
            ReflectionHelper.setPrivateValue(EntityLiving.class, dragon, new DragonBodyHelper(dragon), fieldIndex);
        } catch (Exception ex) {
    ...
    public void onLivingUpdate() {
        ...
        if (mc.isSingleplayer() && mc.thePlayer != null && tameable.isTamed()) {
            String ownerActual = tameable.getOwnerName();
            String ownerExpected = mc.thePlayer.username;
            String entity = CustomEntityList.getEntityString(tameable);
            
            if (!ownerActual.equals(ownerExpected)) {
                L.log(Level.WARNING, "{0} {1} with unexpected owner! Found: \"{2}\", expected: \"{3}\"",
                        new Object[]{entity, tameable.entityId, ownerActual, ownerExpected});
                tameable.setOwner(ownerExpected);
            }
        }
    }
}