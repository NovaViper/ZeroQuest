...
public class ItemCage extends Item{
	...
	public boolean itemInteractionForEntity(ItemStack par1ItemStack, EntityPlayer par2EntityPlayer, EntityLivingBase par3EntityLivingBase)
	{
		...
		if(par1ItemStack.getTagCompound().hasKey(ModEntities.tag) && (!par1ItemStack.getTagCompound().getString(ModEntities.tag).equals("none"))){
			if(!par2EntityPlayer.worldObj.isRemote)
				par2EntityPlayer.addChatMessage("The Animal Trap still has a " + par1ItemStack.getTagCompound().getString(ModEntities.tag) + " inside !");
			return false;
		}else{
			if(par3EntityLivingBase instanceof EntityAnimal){
				...
				nbt = animalCompound;
				// add custom tag
				nbt.setString(ModEntities.tag, CustomEntityList.getEntityString(par3EntityLivingBase));
				...
				if(!par2EntityPlayer.worldObj.isRemote)
					par2EntityPlayer.addChatMessage("Stored a " + CustomEntityList.getEntityString(par3EntityLivingBase) + " in the Animal trap");
